...
axh.set_xticklabels(BENCHES, rotation=28, ha='right', fontsize=10.5)
axh.set_yticks(range(len(UQ)))
axh.set_yticklabels([LABEL[a] for a in UQ], fontsize=11)
for i in range(data.shape[0]):
    for j in range(data.shape[1]):
        v = data[i, j]
        if np.isfinite(v):
            axh.text(j, i, f"{v:.2f}", ha='center', va='center', fontsize=9.5,
